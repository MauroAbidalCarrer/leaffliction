...
def add_graph_data(parent_path, data):
	...
	plants_nbr = sum(1 for entry in os.scandir(parent_path) if entry.is_file() and is_valid_extension(entry.path))
	...
	for elem in child_entries:
		...
		if os.path.isdir(child_path):
			if not os.access(child_path, os.R_OK):
				print(f"Error: Can't access: {child_path}, Read access Denied.")
...
if __name__ == "__main__":
	...
	# Data Dictionary used to add data categories and count valid images per category
	data = {
		"categories": [],
		"nbr": []
	}
